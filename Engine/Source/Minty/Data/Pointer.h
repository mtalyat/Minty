...
#pragma region Methods
    ...
    public:
        ...
        {
            return Unique<T, Allocator>(Memory::Construct(std::forward<Args>(args)...));
        }
...
#pragma region Constructors
    ...
    public:
            ...
            : mp_ptr(ptr), mp_counter(nullptr)
        {
            if (ptr)
            {
                mp_counter = Allocator::construct<PointerCounter>(1, 0);
            }
        }
...
#pragma region Methods
        ...
        {
            return Shared<T, Allocator>(Allocator::construct<T>(std::forward<Args>(args)...));
        }
...
#pragma region Constructors
    ...
    public:
        ...
        explicit Ref(T *const ptr, PointerCounter *const counter)
            : mp_ptr(ptr), mp_counter(counter)
        {
            MINTY_ASSERT((ptr == nullptr && counter == nullptr) || (ptr != nullptr && counter != nullptr), ErrorCode::Argument_Invalid);
            if (mp_counter)
            {
        ...
        Ref(Ref<U> const &other)
            : mp_ptr(static_cast<T*>(other.get())), mp_counter(other.mp_counter)
        {
            if (mp_counter)
            {
                ++mp_counter->weakCount;
