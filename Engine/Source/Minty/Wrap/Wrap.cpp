...
void Minty::Wrap::load(Path const& path)
{
    ...
    if (memcmp(m_header.id, WRAP_MAGIC, WRAP_MAGIC_SIZE))
    {
        ...
        MINTY_ERROR_F(ErrorCode::Wrap_InvalidFormat, m_path.get_string());
        m_header = Header();
        return;
    }
...
uint32_t Minty::Wrap::add_entry(Entry& newEntry)
{
    ...
    // cannot fit
    MINTY_ERROR_F(ErrorCode::Wrap_EntryLimitReached, m_entries.get_size());
    return -1;
}
...
void Minty::Wrap::add(Path const& physicalPath, Path const& virtualPath, CompressionLevel compressionLevel, uint32_t const reservedSize)
{
    ...
    if (static_cast<Bool>(compressionLevel))
    {
        ...
        {
            MINTY_ERROR_F(ErrorCode::Wrap_CompressionFailed, physicalPath.get_string());
            return;
        }
...
Vector<Byte> Minty::Wrap::read_bytes(Path const& path) const
{
	...
	else
	{
		...
		if (!uncompressResult)
		{
			MINTY_ERROR_F(ErrorCode::Wrap_UncompressionFailed, path.get_string());
            delete[] data;
			return Vector<Byte>();
		}
	}
