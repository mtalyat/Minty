...
#ifdef MINTY_DEBUG
#define VK_ASSERT_RESULT(operation, message) do { VkResult result = (operation); if (result != VK_SUCCESS) { MINTY_ABORT_F(ErrorCode::PlatformSpecific, message); }} while (false)
...
#endif // MINTY_DEBUG
#define VK_ASSERT_ABORT(message) MINTY_ABORT_F(ErrorCode::PlatformSpecific, message)
