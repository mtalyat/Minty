...
mFileHandle(fp)
{
	...
	result DiskFile::open(const char *aFilename)
	{
		...
		errno_t err = fopen_s(&mFileHandle, aFilename, "rb");
		if (!mFileHandle)
...
extern "C"
{
	...
	Soloud_Filehack * Soloud_Filehack_fopen(const char *aFilename, char const * /*aMode*/)
	{
	...
	int Soloud_Filehack_fopen_s(Soloud_Filehack** f, const char* aFilename, char const* /*aMode*/)
	{
