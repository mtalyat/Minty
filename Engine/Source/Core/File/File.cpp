...
{
	PhysicalFile file;
	if(!file.open(path, FileFlagsEnum::Read))
	{
...
{
	PhysicalFile file;
	if (!file.open(path, FileFlagsEnum::Write))
	{
		return false;
	}
...
{
	PhysicalFile file;
	if (!file.open(path, FileFlagsEnum::Write | FileFlagsEnum::Binary))
	{
		return false;
	}
...
Vector<Byte> Minty::File::read_bytes()
{
	...
	// read bytes
	set_position(0, StreamDirectionEnum::Begin);
