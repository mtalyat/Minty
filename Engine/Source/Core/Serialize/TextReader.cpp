...
Minty::TextReader::TextReader(Unique<Stream> &&stream)
    : Reader(std::move(stream)), m_nextKey(), m_nextIndent(0), m_hasNextKey(false), m_hasNextValue(false)
{
}
...
// TODO: why are these pointers? Should be references
...
Bool Minty::TextReader::read_tuple(Pointer const buffer, Size const elementSize, UInt const count, Function<Bool(Pointer)> const &readFunc)
{
    Char openingBracket;
    if (!peek(openingBracket) || (openingBracket != '[' && openingBracket != '('))
    {
        MINTY_ERROR(ErrorCodeEnum::Serialization_InvalidFormat);
        return false;
    }
    ...
    for (UInt i = 0; i < count; ++i)
    {
        ...
        // Read the element
        Pointer const elementPtr = static_cast<Byte *>(buffer) + (i * elementSize);
...
Bool Minty::TextReader::read_raw_value(String &value, Bool const ignoreSeparator)
{
    ...
    if (index != INVALID_SIZE && index + 1 < builder.get_length())
    {
...
Bool Minty::TextReader::read_type_value_pair(TypeEnum &type, Pointer data)
{
    StringBuilder builder;
    ...
    if (!Parser<TypeEnum>::parse(typeString, type))
    {
        return false;
    }
    ...
    if (!peek(ch) || ch != ':')
    {
        // If no separator, that is okay- the value will default
        return get_typed_default_value(type, data);
    }
    read_from_stream(&ch, sizeof(Char));
    if (!peek(ch) || ch != ' ')
    {
        // Separator but no space? Okay buddy. Whatever, I don't even care. Just return false.
        return false;
    }
    read_from_stream(&ch, sizeof(Char));
...
Bool Minty::TextReader::check_for_indentation(UInt &indentLevel)
{
    ...
    MINTY_CHECK(!(usedTabs && usedSpaces), ErrorCodeEnum::Serialization_InconsistentIndentation); // Cannot mix tabs and spaces for indentation
    if (usedSpaces)
    {
        // if used spaces, each indent level is 4 spaces
        MINTY_CHECK(indentLevel % 4 == 0, ErrorCodeEnum::Serialization_InconsistentIndentation); // Indentation level must be a multiple of 4 when using spaces
        indentLevel /= 4;
    }
...
void Minty::TextReader::skip_remaining()
{
    while (get_next_key(m_nextKey))
    {
        ...
        if (get_validation())
        {
            ...
            MINTY_WARNING_B(ErrorCodeEnum::Serialization_IgnoredData, get_indent(), m_nextKey.get_data());
        }
