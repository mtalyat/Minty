    ...
    : mp_data(nullptr), m_size(view.get_size()), m_allocator()
{
    ...
    mp_data = m_allocator.allocate<Char>(m_size + 1);
    MINTY_ASSERT_A(mp_data != nullptr, ErrorCodeEnum::Memory_AllocationFailed, sizeof(Char) * (m_size + 1));
...
String Minty::String::operator+(StringView const other) const
{
    ...
    result.mp_data = result.m_allocator.allocate<Char>(result.m_size + 1);
    MINTY_ASSERT_A(result.mp_data != nullptr, ErrorCodeEnum::Memory_AllocationFailed, sizeof(Char) * (result.m_size + 1));
...
String Minty::String::trim_start(StringView const chars) const
{
    ...
    return sub(startIndex, INVALID_SIZE);
}
