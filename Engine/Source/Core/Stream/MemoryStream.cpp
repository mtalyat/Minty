...
void MemoryStream::write(PointerConst const data, Size const size)
{
    ...
    if (requiredSize > m_container->get_capacity())
    {
        ...
        MINTY_ASSERT(reserved, ErrorCodeEnum::Memory_AllocationFailed);
    }
...
Bool MemoryStream::read(Pointer data, Size const size)
{
...
void Minty::MemoryStream::set_position(StreamPosition const position, StreamDirectionEnum const direction)
{
    switch (direction)
    {
        case StreamDirectionEnum::Begin:
            MINTY_ASSERT(position >= 0 && position <= static_cast<StreamPosition>(m_container->get_size()), ErrorCodeEnum::Argument_OutOfRange);
            m_position = position;
            break;
        case StreamDirectionEnum::Current:
            MINTY_ASSERT(m_position + position <= static_cast<StreamPosition>(m_container->get_size()) && m_position + position >= 0, ErrorCodeEnum::Argument_OutOfRange);
            m_position += position;
            break;
        case StreamDirectionEnum::End:
            MINTY_ASSERT(position <= 0 && static_cast<StreamPosition>(m_container->get_size()) + position >= 0, ErrorCodeEnum::Argument_OutOfRange);
...
Char MemoryStream::peek()
{
    ...
    PointerConst const ptr = m_container->get_at(m_position);
