...
Bool Minty::Serializer<SpriteComponent>::serialize(Writer &writer, SpriteComponent const &value)
{
...
Bool Minty::Serializer<SpriteComponent>::deserialize(Reader &reader, SpriteComponent &value)
{
	...
	if (reader.read_primary("Sprite", id))
	{
		...
		MINTY_ASSERT_F(value.source != nullptr, ErrorCodeEnum::Asset_MissingDependency, id);
	}

	if(value.source != nullptr)
	{
		AssetType assetType = value.source->get_asset_type();
		MINTY_ASSERT_F(assetType == AssetType::Sprite || assetType == AssetType::SpriteAtlas, ErrorCodeEnum::Asset_InvalidDependencyType, id);
